...
只有推送 `v*` 版本 tag 时，GitHub Actions 才会构建并推送镜像，例如 `v1.0.5`：
...
如果 GHCR package 是私有的，服务器需要使用带 `read:packages` 权限的 GitHub PAT 登录：
...
先确认 package 是否私有。如果是私有包，使用带 `read:packages` 权限的 GitHub PAT 登录：
